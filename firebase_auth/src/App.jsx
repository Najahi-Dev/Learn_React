      ...
      ${formVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'} transform transition-all duration-500 ease-out`}>
        ...
        <form onSubmit={(e) => handleSubmit(e, setError)} className='space-y-6'>
          <div>
            ...
            <input type="email" name='email' id='email' placeholder='Enter Your Email' required className='w-full border-b border-gray-600 
            bg-transparent text-white px-2 py-1 focus:border-cyan-400 focus:outline-none'/>
          </div>

          <div className='relative'>
            <label htmlFor="password" className='block text-gray-300 font-medium mb-1'>Password</label>
            <input type={passwordVisible ? 'text' : 'password'} name='password' id='password' placeholder='Enter Your Password' required className='w-full border-b border-gray-600 
            bg-transparent text-white px-2 py-1 focus:border-cyan-400 focus:outline-none'/>
          ...
          {/* Login Button */}
          <button type='submit' className='w-full bg-gradient-to-r from-cyan-500 to-blue-500 text-white py-2 rounded-lg 
          hover:bg-gradient-to-l hover:to-cyan-600 transition-all duration-300 focus:ring focus:ring-cyan-300 
          focus:outline-none shadow-md hover:shadow-lg'>Login</button>
        </form>
        ...
        {/* Google Button */}
        <button onClick={() => handleGoogleLogin(setError)} className='mt-6 w-full flex items-center justify-center bg-gray-700 border-gray-600 py-2 
